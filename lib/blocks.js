...
BlockController.prototype.getBlockByHash = function (hash, next) {
    ...
    if (blockCached) {
        ...
        blockCached.confirmations = self.node.services.luxd.height - blockCached.height + 1;
        ...
        return async.waterfall([function (callback) {
            ...
            return self.node.services.luxd.getBlockHeader(hash, function (err, info) {
...
BlockController.prototype.blockIndex = function (req, res) {
    ...
    this.node.services.luxd.getBlockHeader(parseInt(height), function (err, info) {
        if (err) {
            return self.common.handleErrors(err, res);
        }
        res.jsonp({
...
BlockController.prototype._getBlockSummary = function (hash, moreTimestamp, next) {
    ...
    if (summaryCache) {
        ...
        return async.waterfall([function (callback) {
            return self.node.services.luxd.client.getBlock(hash, function (err, response) {
            ...
            if (block.flags == bitcore.Block.PROOF_OF_WORK) {
                txHash = block.tx[0];
            } else {
                txHash = block.tx[1];
            }
            ...
            var confirmations = self.node.services.luxd.height - block.height + 1;
...
BlockController.prototype.list = function (req, res) {
    ...
    return self.node.services.luxd.getBlockHashesByTimestamp(lte, gte, function (err, hashes) {
