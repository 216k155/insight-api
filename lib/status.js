...
StatusController.prototype.show = function(req, res) {
  ...
  case 'getStakingStatus':
      return self.getStakingStatus(function(err, result) {
...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.luxd.getInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getMiningInfo = function(callback) {
    return this.node.services.luxd.getMiningInfo(function(err, result) {
...
StatusController.prototype.getStakingStatus = function(callback) {
    return this.node.services.luxd.getStakingStatus(function(err, result) {
...
StatusController.prototype.getDgpInfo = function(req, res) {
  ...
  return this.node.services.luxd.getDgpInfo(function(err, result) {
...
StatusController.prototype.getLastBlockHash = function() {
  var hash = this.node.services.luxd.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function(callback) {
  this.node.services.luxd.getBestBlockHash(function(err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.luxd.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.luxd.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.luxd.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.luxd.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.luxd.height,
        error: null,
        type: 'luxcore node'
      };
